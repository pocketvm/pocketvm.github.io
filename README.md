...
A static HTML virtual machine runner for GitHub Pages. No build step, backend, account, or API key required.

## Publish on GitHub Pages

1. Create a GitHub repository, or open an existing one.
2. Upload **the contents of this folder** to the repository root. Keep `vendor/` and its files together. `index.html` must be at the root, not inside another `vm-runner/` folder.
3. Open **Settings → Pages**. Under **Build and deployment**, select **Deploy from a branch**.
4. Choose your branch (usually `main`) and **/ (root)**, then Save.
5. Open the site link GitHub provides when deployment finishes.

Relative asset paths support both account sites and repository sites. Include `.nojekyll` if uploading with Git; it is optional for the other files in this package.
...
Local images are read in browser memory and are never uploaded. The page caps local images at 512 MB to limit memory pressure. Guest disk changes are temporary: there is no automatic saving or disk export. No guest networking relay is configured. This emulates a 32-bit x86 PC; it is not a modern 64-bit VM host. A desktop browser with a physical keyboard works best.
...
- `vendor/libv86.js`, `v86.wasm`, `v86-fallback.wasm`: v86 0.5.458 from https://www.npmjs.com/package/v86/v/0.5.458. BSD-2-Clause license in `vendor/V86-LICENSE`. Source: https://github.com/copy/v86.
...
Verification: JavaScript syntax checked; emulator boot tested separately using the bundled firmware and Linux demo. Browser UI interaction has not been automatically tested.
